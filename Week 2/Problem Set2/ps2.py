    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(new_position):
            ...
            while (not self.room.isPositionInRoom(new_position)):
                self.direct = random.randrange(360)
                new_position = Position(self.getRobotPosition().getX(), self.getRobotPosition().getY()).getNewPosition(
                    self.direct, self.speed)
                # new_position=Position(pos.getX(),pos.getY()).getNewPosition(self.direct, self.speed)
                # self.setRobotPosition(new_position)
        '''
...
# runSimulation(2, 2.0, 10, 10, 0.75, 30, StandardRobot)
...
# ###################################
# My fantasy code
def visualize(type_robot):
    anim = ps2_visualize.RobotVisualization(1, 20, 20)
    room = RectangularRoom(anim.width, anim.height)
    robots = {}
    speed = 1
    for num in range(anim.num_robots):
        robots[num] = type_robot(room, speed)
        print room.getNumCleanedTiles(),
        robot_list = robots.values()

    while (room.getNumTiles() != room.getNumCleanedTiles()):
        anim.update(room, robot_list)
        for num in range(anim.num_robots):
            robots[num] = StandardRobot(room, speed)
            print room.getNumCleanedTiles(),
            robot_list = robots.values()


visualize(StandardRobot)
