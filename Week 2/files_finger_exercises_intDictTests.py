...
def insertions_size_finder(probability):

    numBuckets=1000
    lo_numInsertions=30
    hi_numInsertions=250
    numInsertions=int((lo_numInsertions+hi_numInsertions)/2)
    epsilon=1
    a=0
    b=0
    getcontext().prec=2
    while True:
        a+=1
        b+=1
        #print 'baslangic ' + str(a)
        #print numInsertions
        coll_prob=collision_prob(numBuckets,numInsertions)
        coll_prob=round(coll_prob,2)#Decimal(coll_prob)/Decimal(1)
        #print '\n ' + str(coll_prob) + '\n'
        if coll_prob<probability:
            return 
        else:
           # print "burda " + str(b)
            if coll_prob<probability:
                lo_numInsertions=numInsertions
            elif coll_prob>probability:
                hi_numInsertions=numInsertions
